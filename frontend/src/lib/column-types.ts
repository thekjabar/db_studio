...
export const PG_COLUMN_TYPES: { group: string; items: { value: string; label?: string }[] }[] = [
  {
    group: "Text",
    items: [
      { value: "text" },
...
export function pgTypeOptions(currentValue?: string): { value: string; label?: string }[] {
  const flat = PG_COLUMN_TYPES.flatMap((g) =>
    g.items.map((i) => ({ value: i.value, label: `${g.group} · ${i.label ?? i.value}` })),
  );
