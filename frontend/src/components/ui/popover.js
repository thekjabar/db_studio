 ...
 */
export function Popover({ open, onOpenChange, anchorRef, align = "start", side = "auto", children, className, }) {
    ...
    React.useLayoutEffect(() => {
        ...
        if (align === "end") {
            ...
            if (contentW && leftIfEnd < margin)
                s.left = margin;
            else
                s.right = rightFromRight;
        }
    ...
    React.useEffect(() => {
        ...
        const onDown = (e) => {
            ...
            if (t.closest("[data-radix-popper-content-wrapper]"))
                return;
            if (t.closest("[data-radix-portal]"))
                return;
    ...
    return createPortal(_jsx("div", { ref: ref, style: style, "data-popover-content": "", onPointerDownCapture: (e) => e.stopPropagation(), className: cn("rounded-md border border-border bg-card shadow-xl p-2", className), children: children }), document.body);
}
