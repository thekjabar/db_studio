...
// `@monaco-editor/react` exposes the full monaco namespace as `Monaco`.
// We only need a disposable return value; avoid pulling types from
// `monaco-editor` (optional peer, may not be installed).
  ...
  return monaco.languages.registerCompletionItemProvider("sql", {
    ...
    provideCompletionItems(model: any, position: any) {
      ...
      const inTableContext =
        /\b(FROM|JOIN|INTO|UPDATE|DELETE FROM)\s+$/.test(prefix) ||
