...
export function EditConnectionDialog({ connection, onOpenChange }) {
    ...
    const submit = (e) => {
        ...
        if (msNum !== (connection.slowQueryAlertMs ?? null))
            patch.slowQueryAlertMs = msNum;
        const emailVal = alertEmail.trim() || null;
        if (emailVal !== (connection.slowQueryAlertEmail ?? null))
            patch.slowQueryAlertEmail = emailVal;
